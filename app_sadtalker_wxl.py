        ...
        with gr.Row().style(equal_height=False):
            with gr.Column(variant='panel'):
                with gr.Tabs(elem_id="sadtalker_source_image"):
                    with gr.TabItem('源图像'):
                        with gr.Row():
                            source_image = gr.Image(label="上传源图像", source="upload", type="filepath", elem_id="img2img_image").style(width=512)


                with gr.Tabs(elem_id="sadtalker_driven_audio"):
                    with gr.TabItem('生成方式'):
                        gr.Markdown("可供选择的生成方式: <br> 1. 仅音频 2. 音频+参考视频 3. IDLE模式 4.仅参考视频")

                        with gr.Row():
                            driven_audio = gr.Audio(label="输入音频", source="upload", type="filepath")
                        ...
                        with gr.Row():
                            ref_video = gr.Video(label="参考视频", source="upload", type="filepath", elem_id="vidref").style(width=512)

                            with gr.Column():
                                use_ref_video = gr.Checkbox(label="参考视频生成")
                                ref_info = gr.Radio(['pose', 'blink','pose+blink', 'all'], value='pose', label='参考视频',info="如何参考视频？")
                ...
                with gr.Tabs(elem_id="sadtalker_checkbox"):
                    with gr.TabItem('设置'):
                        with gr.Column(variant='panel'):
                            ...
                            with gr.Row():
                                is_still_mode = gr.Checkbox(label="静止模式(仅工作在`full`下)")
                                facerender = gr.Radio(['facevid2vid','pirender'], value='facevid2vid', label='视频合成方式(质量模式、速度模式)')
