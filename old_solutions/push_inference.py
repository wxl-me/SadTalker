...
def main(args):
    ...
    CHANNELS = 2
    CHUNK = 4410
    stream = p.open(format=pyaudio.paInt16, channels=CHANNELS, rate=RATE, input=True, input_device_index=-1, frames_per_buffer=CHUNK)
    frames = []
    rms_window = []
    def record_thread(n):
        while True:
            ...
            #rms_window.append(audioop.rms(data))
            if len(frames)>n:
                frames.pop(0)
    threading.Thread(target=record_thread,args=(RATE//CHUNK*interrupt_t,)).start()
    time.sleep(interrupt_t)
    last_t = time.time()
    ...
    while 1:
        ...
        wf = wave.open(seg_audio_path, 'wb')	#保存
        ...
        #audio2ceoff
        batch = get_data(first_coeff_path, seg_audio_path, device, ref_eyeblink_coeff_path, still=args.still)
        coeff_path = audio_to_coeff.generate(batch, save_dir, pose_style, ref_pose_coeff_path)
        
        #coeff2video
        data = get_facerender_data(coeff_path, crop_pic_path, first_coeff_path, seg_audio_path, 
